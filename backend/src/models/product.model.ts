...
// Virtual field: availableQuantity (safe version)
productSchema.virtual("availableQuantity").get(function (this: IProduct) {
  if (!this.inventory) return 0; // prevent undefined crash
  return Math.max(0, (this.inventory.quantity || 0) - (this.inventory.reserved || 0));
});

// Virtual field: isLowStock (safe version)
productSchema.virtual("isLowStock").get(function (this: IProduct) {
  if (!this.inventory) return false; // prevent undefined crash
  return this.availableQuantity <= (this.inventory.lowStockThreshold || 10);
});
